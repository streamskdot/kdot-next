...
export function ExoclickMobileMatchCardBanner({ className = '', onAdLoadError }: ExoclickMobileMatchCardBannerProps & { onAdLoadError?: () => void }) {
  ...
  useEffect(() => {
    ...
    if (document.querySelector('script[src="https://a.magsrv.com/ad-provider.js"]') || scriptLoaded) {
      ...
      script.onerror = () => {
        console.log('Exoclick mobile match card banner script blocked by privacy protection')
        if (onAdLoadError) {
          onAdLoadError()
        }
      }
