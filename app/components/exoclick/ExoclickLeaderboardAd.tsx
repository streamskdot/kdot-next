...
interface ExoclickLeaderboardAdProps {
  className?: string
}
...
export function ExoclickLeaderboardAd({ className = '', onAdLoadError }: ExoclickLeaderboardAdProps & { onAdLoadError?: () => void }) {
  ...
  useEffect(() => {
    ...
    if (document.querySelector('script[src="https://a.magsrv.com/ad-provider.js"]') || scriptLoaded) {
      ...
      script.onerror = () => {
        console.log('Exoclick leaderboard script blocked by privacy protection')
        if (onAdLoadError) {
          onAdLoadError()
        }
      }
