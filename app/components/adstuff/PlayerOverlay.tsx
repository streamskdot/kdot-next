...
/**
 * Overlay with play button UI that triggers hilltop popunders on first click.
 * After 300ms, overlay becomes transparent and allows iframe interaction.
 * Sandbox attribute on iframe blocks the provider's own popunders.
 */
...
export const PlayerOverlay = forwardRef<HTMLDivElement, PlayerOverlayProps>(
  ({ onFirstClick, visible = true }, ref) => {
    ...
    const { armPopunder, disarmPopunder } = useHilltopPopunder(visible && !hasClicked)

    // Arm the popunder while the overlay is visible and not yet clicked.
    useEffect(() => {
      if (!visible || hasClicked) {
        disarmPopunder()
        return
      }
      armPopunder()
      return () => disarmPopunder()
    }, [visible, hasClicked, armPopunder, disarmPopunder])
    ...
    return (
      <div
        ...
        className="absolute inset-0 z-10 transition-opacity duration-300 group"
        style={{
          cursor: 'pointer',
