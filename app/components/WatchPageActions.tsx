...
export function WatchPageActions({ matchId }: WatchPageActionsProps) {
  const [copied, setCopied] = useState(false)

  const handleCopyLink = () => {
    const url = matchId ? `${window.location.origin}/match/${matchId}` : window.location.href
  ...
  return (
    <div className="flex items-center gap-1">
      ...
      <a
        ...
        target="_blank"
        rel="noopener noreferrer"
        className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-green-500 hover:bg-green-600 text-white transition-colors"
      >
        <svg className="h-4 w-4" fill="currentColor" viewBox="0 0 24 24">
      ...
      <button
        ...
        className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-zinc-200 hover:bg-zinc-300 dark:bg-zinc-700 dark:hover:bg-zinc-600 text-zinc-700 dark:text-zinc-300 transition-colors"
      >
